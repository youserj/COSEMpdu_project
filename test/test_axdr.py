...
from src.COSEMpdu.x680.type import INTEGER, SizeConstraint, ValueRange, Constraint
...
from src.COSEMpdu.x690 import Tag
...
class TestIntegerType(unittest.TestCase):
    ...
    def test_encode_fixed_length_1byte(self):
        """Fixed-length INTEGER(0..255) encodes as 1 byte (§6.1.1)"""
        @dataclass
        class OneByte(IntegerType):
            fixed_length = 1

        val = IntegerType(value=42)
        buf = ByteBuffer.allocate(1)
        written = val.put(buf)
        self.assertEqual(written, 1)
        self.assertEqual(bytes(buf), b'\x2A')
    
    def test_encode_fixed_length_2bytes(self):
        """Fixed-length INTEGER(0..65535) encodes as 2 bytes"""
        @dataclass
        class TwoByte(IntegerType):
            fixed_length = 2

        val = TwoByte(value=12345)
        buf = ByteBuffer.allocate(2)
        written = val.put(buf)
        self.assertEqual(written, 2)
        self.assertEqual(bytes(buf), b'\x30\x39')
    
    def test_encode_variable_short(self):
    ...
    def test_encode_variable_long(self):
    ...
    def test_decode_fixed_length(self):
        """Decode fixed-length INTEGER"""
        @dataclass
        class TwoByte(IntegerType):
            constraint = Constraint(ValueRange(0, 65535))
...
class TestBitStringType(unittest.TestCase):
    ...
    def test_encode_fixed_length(self):
        ...
        @dataclass
        class OctetBit(BitStringType):
            constraint = Constraint(SizeConstraint(8))

        val = OctetBit(value=(1, 0, 1, 1, 0, 0, 1, 1))
        buf = ByteBuffer.allocate(1)
        written = val.put(buf)
        self.assertEqual(written, 1)
        self.assertEqual(bytes(buf), b'\xB3')
    
    def test_encode_fixed_length_padded(self):
        """Fixed-length BIT STRING pads to octet boundary"""
        @dataclass
        class FourBit(BitStringType):
            constraint = Constraint(SizeConstraint(4))

        val = FourBit(value=(1, 0, 1, 1))
        buf = ByteBuffer.allocate(1)
        written = val.put(buf)
        self.assertEqual(written, 1)
    ...
    def test_decode_variable_length(self):
        ...
        self.assertEqual(val.constraint, None)
    
    def test_encode_empty_fixed(self):
        """Empty fixed-length BIT STRING encodes nothing"""
        @dataclass
        class Empty(BitStringType):
            constraint = Constraint(SizeConstraint(0))
    ...
    def test_encode_empty_variable(self):
...
class TestOctetStringType(unittest.TestCase):
    ...
    def test_encode_fixed_length(self):
        ...
        @dataclass
        class Octet(OctetStringType):
            constraint = Constraint(SizeConstraint(4))

        val = Octet(value=b'ABCD')
        buf = ByteBuffer.allocate(4)
        written = val.put(buf)
...
class TestChoiceType(unittest.TestCase):
    ...
    def setUp(self):
        """Set up test CHOICE type with alternatives"""
        self.ChoiceClass = TestChoice
    
    def test_encode_integer_alternative(self):
        """CHOICE with INTEGER alternative encodes tag + content (§6.6)"""
        val = self.ChoiceClass.from_id("first", IntegerType(value=42))
        buf = ByteBuffer.allocate(3)
        written = val.put(buf)
    ...
    def test_encode_octetstring_alternative(self):
        ...
        val = self.ChoiceClass.from_id("second", OctetStringType(value=b'AB'))
...
class TestSequenceType(unittest.TestCase):
    ...
    def test_encode_with_optional_present(self):
    ...
    def test_encode_with_optional_absent(self):
    ...
    def test_encode_with_default_present(self):
    ...
    def test_encode_with_default_absent(self):
    ...
    def test_decode_with_optional_present(self):
    ...
    def test_decode_with_optional_absent(self):
...
class TestIntegration(unittest.TestCase):
    ...
    def test_sequence_of_choice(self):
        """Test SEQUENCE OF CHOICE (DLMS service list pattern)"""
        @dataclass
        class Enumerated0(TaggedType):
            tag = 0
            mode = x680.TaggingMode.IMPLICIT
            type_ = EnumeratedType

        @dataclass
        class Integer1(TaggedType):
            tag = 1
            mode = x680.TaggingMode.IMPLICIT
            type_ = IntegerType
